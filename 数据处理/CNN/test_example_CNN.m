...
% test_y = double(test_y');
 ...
 load F:\毕业相关\大论文\GraduateWork\数据处理\数据与结果\直管\9Mpa\8\data  %数据
 load F:\毕业相关\大论文\GraduateWork\数据处理\数据与结果\直管\9Mpa\8\label %标签
%  l1=zeros(525,1);l2=ones(525,1);
%  label=[l2 l1;l1 l2];
%  data=data([1:525 3151:3675],:);
%   data=data([1:1050 1576:2100 3151:3675],:);
   data=data([1:1750 3501:10500],:);
   l1=zeros(1750,1);l2=ones(1750,1);
  label=[l2 l1 l1 l1 l1;l1 l2 l1 l1 l1;l1 l1 l2 l1 l1;l1 l1 l1 l2 l1;l1 l1 l1 l1 l2;];
 Err=[];
 trainerr=[];
%%定义好训练集与测试集数量
rowNum=size(data,1);
trainSize=6000;  %训练集数量
...
train_x = double(reshape(train_data',8,8,trainSize))/255;
test_x = double(reshape(test_data',8,8,testSize))/255;
